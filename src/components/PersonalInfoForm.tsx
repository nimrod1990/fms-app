...
const PersonalInfoForm: React.FC<PersonalInfoFormProps> = ({ onSubmit }) => {
  ...
  return (
    <form className="personal-info-form flex" onSubmit={handleSubmit}>
      <h2 className="m-[20px]">红医师运动伤风险评估</h2>
      ...
      <div className="form-group">
        ...
        <select
          ...
          value={dominantFoot}
          onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
            setDominantFoot(e.target.value as DominantFoot)
          }
        >
      ...
      <button
        type="submit"
        className="p-[12px] px-[24px] py-[12px] h-full w-full"
      >
        开始测试
      </button>
    </form>
